...
const title = computed(() => {
    return editMode.value ? 'Edit Trade' : 'Add Trade';
})

onMounted(() => {
    const params = dialogRef!.value.data;
    editMode.value = !params.new;
    if (params.new) {
        form.instrument_id = dialogRef!.value.data.instrument_id;
        form.defaults();
    } else {
        form.defaults(params.trade);
        form.reset();
    }
})
const closeDialog = () => {
...
const submit = () => {
    const url = editMode.value ? route('trades.update', {trade: form.id!}) : route('trades.store');
    const _method = editMode.value ? 'patch' : 'post';
    form.submit(_method, url, {
        onSuccess: (res) => {
        ...
        <Panel :pt="{ header: 'p-2 px-4' }">
            <template #header>
                <div class="flex items-center gap-2">
                    <h2 class="text-lg font-medium uppercase">{{ title }}</h2>
                </div>
            </template>
            <div class="m-0">
