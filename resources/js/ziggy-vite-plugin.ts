...
export default function ZiggyPlugin(config: ZiggyPluginConfig): Plugin {
    ...
    const commandArgs = `artisan ziggy:generate ${destination ?? ''} ${declarations ? (declarations === 'only' ? '--types-only' : '--types') : ''}`;
    ...
    return {
        ...
        configureServer(server) {
            ...
            const checkReload = (path: any) => {
                if (shouldReload(path)) {
                    setTimeout(command, delay);
                    if (log) {
                        server.config.logger.info(
                            `${colors.green(`${relative(root, path)} changed, regenarating ziggy file.`)}`,
                            {
                                clear: true,
                                timestamp: true,
                            }
